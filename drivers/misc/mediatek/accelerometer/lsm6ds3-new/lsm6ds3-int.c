...
#define LSM6DS3_CALIB_HAL                             // CALIB FROM st HAL
/*st-sw caoyi1 add for Gsensor APK calibration 20150512 end*/
...
/*st-sw caoyi1 modify for GPIO pin 20150325 begin*/
...
/*st-sw caoyi1 modify for GPIO pin 20150325 end*/
...
/* st-sw liaoxl add for int2 pin of lsm6ds3  20150505 start */
...
/*st-sw caoyi1 modify A+G sensro iic address 20150417 begin*/
//static struct i2c_board_info __initdata i2c_lsm6ds3={ I2C_BOARD_INFO(LSM6DS3_ACC_DEV_NAME, (0xD5>>1))};
/*st-sw caoyi1 modify A+G sensro iic address 20150417 end*/
...
static bool sigm_enable_status = false;  /* add for significant motioon sensor -- by liaoxl.st 5.12.2015 */
//static bool stepd_enable_status = false; /* add for step detector sensor -- by liaoxl.st 7.12.2015 */
...
struct mutex lsm6ds3_mutex;
DEFINE_MUTEX(lsm6ds3_mutex);
...
{
/*st-sw caoyi1 modify for debug 20150325 begin*/
	int i=0;
	u8 addr = 0x0C;
	u8 regdata=0;
...
//	u8 databuf[2] = {0};    
	...
	{	
		...
		/* change reg value from 0x1B to 0x16 for step counter debounce on bus env --modified by liaoxl.st 7.6.2015 */
...
/* remove step counter reset operation to prevent data losing when system sleeping --modified by liaoxl.st 7.28.2015 start */
		if(obj->boot_deb == 2)
		{
			obj->boot_deb = 1;
...
/* remove step counter reset operation to prevent data losing when system sleeping --modified by liaoxl.st 7.28.2015 end */
...
/*st-sw caoyi1 add for Gsensor APK calibration 20150526 begin*/
 #ifdef LSM6DS3_CALIB_HAL
...
static int lsm6ds3_step_c_enable_step_detect(int en)
{
	...
	if(0 == en)
	{
		...
		stepd_enable_status = false;  /* add for step detector sensor -- by liaoxl.st 7.12.2015 */
	...
	else if(1 == en)
	{
		...
		if(0 == atomic_read(&obj->int1_request_num))
			mt_eint_unmask(CUST_EINT_LSM6DS3_NUM);
		atomic_inc(&obj->int1_request_num);
		stepd_enable_status = true;  /* add for step detector sensor -- by liaoxl.st 7.12.2015 */
...
/* st-sw liaoxl add for reduce power consume of lsm6ds3  20150715 end */
#endif
...
static int lsm6ds3_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	/* st-sw, weimh1, 2016-1-24 add begin: mod real addr*/
	client->addr = 0x6B;
	/* st-sw, weimh1, 2016-1-24 add end: mod real addr*/
...
/* init step counter intterupt vars  --add by liaoxl.st 7.28.2015 start */
	obj->boot_deb = 2;
	obj->overflow = 0;
	obj->steps_c = 0;
	obj->step_c_delay = 3;
...
/* init step counter intterupt vars  --add by liaoxl.st 7.28.2015  end */
...
static int lsm6ds3_local_init(void)
{
	...
	if (-1 == lsm6ds3_acc_init_flag)	{
		GSE_ERR("lsm6ds3_acc_init  driver fail\n");
		return -1;
	}
